...
# Download and install Inveigh
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Downloading Inveigh..."
$InveighDownloadUrl = "https://github.com/Kevin-Robertson/Inveigh/archive/refs/heads/master.zip"
$InveighRepoPath = "C:\Users\vagrant\AppData\Local\Temp\Inveigh.zip"
If (-not (Test-Path $InveighRepoPath)) {
  Invoke-WebRequest -Uri "$InveighDownloadUrl" -OutFile "$InveighRepoPath"
  Expand-Archive -path "$InveighRepoPath" -destinationpath 'c:\Tools\Inveigh' -Force
} Else {
  Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Inveigh was already installed. Moving On."
}
